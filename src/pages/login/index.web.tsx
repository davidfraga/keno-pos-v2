          ...
          <Layout>
            <Card disabled={true} footer={Footer}>
              ...
              <View>
                {/* @ts-ignore */}
                <select
                  value={clients?.findIndex((c) => c.clientName === serverName) >= 0 ? clients.findIndex((c) => c.clientName === serverName) : ""}
                  style={{
                    width: '100%',
                    padding: 12,
                    borderRadius: 5,
                  ...
                  onChange={(e: any) => handleSelectDomain({ row: parseInt(e.target.value) } as any)}
                >
                  ...
                  {clients && clients.map((dominio, index) => (
                    <option key={`std${dominio.id}`} value={index}>
                      {dominio.clientName}
                    </option>
                  ))}
                {/* @ts-ignore */}
              ...
              <View>
                {/* @ts-ignore */}
                <select
                  value={selectedClient?.servers?.findIndex((s) => s.subDomain === server) >= 0 ? selectedClient.servers.findIndex((s) => s.subDomain === server) : ""}
                  style={{
                    width: '100%',
                    padding: 12,
                    borderRadius: 5,
                  ...
                  onChange={(e: any) => handleSelectServer({ row: parseInt(e.target.value) } as any)}
                >
                  <option value="" disabled>
                    {!selectedClient ? 'Selecione um Cliente' : 'Selecione um Server'}
                  </option>
                  {selectedClient && selectedClient.servers?.map((srv, index) => (
                    <option key={`std${srv.id}`} value={index}>
                      {srv.subDomain}
                    </option>
                  ))}
                {/* @ts-ignore */}
