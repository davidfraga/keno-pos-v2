            ...
            <Select
              placeholder={
                searchValue && searchValue != 0
                  ? formatSorteioTitle(sorteios.filter((val) => val.codigo == searchValue)[0])
                  : sorteioSelecionado !== undefined
                  ? formatSorteioTitle(sorteioSelecionado)
                  : sorteios?.length === 0
                ...
                <>
                  {searchValue && searchValue != ''
                    ? sorteios.map((sorteio) => {
                        return (
                          <SelectItem
                            selected={searchValue && searchValue == sorteio.codigo ? true : false}
                            key={`std_${sorteio.codigo}`}
                            title={(evaProps: any) => (
                              <Text
                                {...evaProps}
                                style={{
                                  color: sorteioColor(sorteio)
                                }}
                              >
